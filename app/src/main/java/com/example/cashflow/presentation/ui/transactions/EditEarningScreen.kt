...
@Composable
fun EditEarningScreen(id: Int, onCancelClicked: () -> Unit = {}) {
    ...
    LaunchedEffect(id) {
        val fetchedIncome = viewModel.getIncome(id)
        fetchedIncome?.let { inc ->
            income = inc
            viewModel.title = inc.title
            viewModel.amount = inc.amount.toString()
            viewModel.details = inc.details
            viewModel.selectedCategory = viewModel.getCategory(inc.categoryId)?.title ?: ""
            viewModel.selectedAccount = viewModel.getAccount(inc.accountId)?.title ?: ""
            viewModel.selectedDate = LocalDate.parse(inc.date)
        }
    }
            ...
            CashFlowButton(onClick = {
                if (viewModel.validateForm()) {
                    val updatedIncome = income?.copy(
                        ...
                        amount = viewModel.amount.toDoubleOrNull() ?: return@CashFlowButton,
                        categoryId = viewModel.categories.value.find { it.title == viewModel.selectedCategory }?.id
                            ?: return@CashFlowButton,
                        accountId = viewModel.accounts.value.find { it.title == viewModel.selectedAccount }?.id
                            ?: return@CashFlowButton,
                        date = viewModel.selectedDate.toString()
                    )
                    updatedIncome?.let {
                        viewModel.updateIncome(it)
                        viewModel.showEditIncomeSheet = false
                        Toast.makeText(context, "Income updated successfully", Toast.LENGTH_SHORT)
                            .show()
                        viewModel.resetForm()
                ...
                Text(modifier = Modifier.padding(horizontal = 4.dp), text = "Update Income")
            }
        }
