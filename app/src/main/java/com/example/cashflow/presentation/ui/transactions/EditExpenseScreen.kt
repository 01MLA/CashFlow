...
@Composable
fun EditExpenseScreen(id: Int, onCancelClicked: () -> Unit = {}) {
    ...
    LaunchedEffect(id) {
        val fetchedExpense = viewModel.getExpense(id)
        fetchedExpense?.let { exp ->
            expense = exp
            viewModel.title = exp.title
            viewModel.amount = exp.amount.toString()
            viewModel.details = exp.details
            viewModel.selectedCategory = viewModel.getCategory(exp.categoryId)?.title ?: ""
            viewModel.selectedAccount = viewModel.getAccount(exp.accountId)?.title ?: ""
            viewModel.selectedDate = LocalDate.parse(exp.date)
        }
    }
            ...
            CashFlowButton(onClick = {
                if (viewModel.validateForm()) {
                    ...
                    updatedExpense?.let {
                        ...
                        viewModel.showEditExpenseSheet = false
                ...
                Text(modifier = Modifier.padding(horizontal = 4.dp), text = "Update Expense")
            }
        }
